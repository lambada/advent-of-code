...
let wire1Coords = calculateWireCoords(wire1)
let wire2Coords = calculateWireCoords(wire2)

let crossingCoords = []
wire1Coords.forEach(coord => {
  wire2Coords.forEach(testCoord => {
    if (coord[0] === testCoord[0] && coord[1] === testCoord[1]) {
      crossingCoords.push(coord)
    }
  })
})

let minimalManhattanCoord = []
let minimalManhattanDistance = undefined
crossingCoords.forEach(coord => {
  const manhattanDistance = Math.abs(0 - coord[0]) + Math.abs(0 - coord[1])
  if (!minimalManhattanDistance || (manhattanDistance < minimalManhattanDistance)) {
    minimalManhattanCoord = coord
    minimalManhattanDistance = manhattanDistance
  }
})

console.log(minimalManhattanDistance)
console.log(minimalManhattanCoord)